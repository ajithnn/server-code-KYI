...
siz = np.shape(b_img)
print siz
scaleDownFactor = int(np.sqrt(np.product(siz)/(600*400*3)))
        ...
        for (ex,ey,ew,eh) in eyes:
            ...
            if np.squeeze([eyecx>0.2*h] and [eyecx<0.5*h]):
                ...
                if (type(circles) == type(a)):
                    ...
                    iris_roi = eye_det_b_img[cenRow-cenRad:cenRow+cenRad+1, cenCol-cenRad:cenCol+cenRad+1, :]
                    #iris_roi2 = iris_roi
                    ...
                    eye_det_b_img[cenRow-cenRad:cenRow+cenRad+1, cenCol-cenRad:cenCol+cenRad+1, :] = iris_roi
